...
- **历史记录管理**：分析结果自动保存，支持查看详情、恢复现场、删除记录，并按登录用户隔离数据。
...
数据库字段和 MySQL 迁移建议见：`docs/database.md`
...
**CareerPilot AI 简历分析与模拟面试平台｜Java 后端开发｜Spring Boot、JPA、H2/Redis、Apache Tika、Vue3、OpenAI 兼容接口**
...
- 使用 JPA 持久化分析记录，保存简历名称、岗位 JD、匹配分数、分析摘要、完整 JSON 结果和创建时间，支持历史列表、详情恢复和删除。
...
- 接入 MySQL，并补充正式索引和部署配置。
- 将 AI 对话历史持久化到数据库。
- 增加每日调用额度和默认 Key 保护策略。
- 增加 Docker Compose 一键启动。
- 部署到云服务器并配置 Nginx 反向代理。
